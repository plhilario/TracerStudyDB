...
FROM 
    tracer_study
WHERE
    SHSStrand IS NOT NULL AND Educational_Status IS NOT NULL AND CollegeGPA IS NOT NULL
GROUP BY 
    SHSStrand, Educational_Status;
...
JOIN (
    ...
    GROUP BY 
        SHSStrand, Educational_Status
) AS Expected ON Observed.SHSStrand = Expected.SHSStrand AND Observed.Educational_Status = Expected.Educational_Status;
...
JOIN 
    Expected_Frequencies AS Expected ON Observed.SHSStrand = Expected.SHSStrand AND Observed.Educational_Status = Expected.Educational_Status;
...
SELECT (COUNT(DISTINCT SHSStrand) - 1) * (COUNT(DISTINCT Educational_Status) - 1) AS Degrees_of_Freedom
...
FROM
    (SELECT SUM((o.observed_frequency - e.expected_frequency) * (o.observed_frequency - e.expected_frequency) / e.expected_frequency) AS chi_square_statistic
     ...
     JOIN expected_frequencies e ON o.SHSStrand = e.SHSStrand) AS chi_square_value;